...
@testset "Execution" begin
    ...
    pcm = PortfolioConstructionModel(broker, "test_port", uni, order_sizer, port_optimizer,
                                     alpha_model)
    ...
    broker = SimulatedBroker(DateTime(2020), SimulatedExchange(DateTime(2020)), dh;
                             initial_cash=100_000.0)
    create_portfolio!(broker, 75_000; portfolio_id="test_port")
    exec_handler = SaguaroTrader.ExecutionHandler(broker, "test_port"; submit_orders=false)
    exec_handler(rebalance_orders)
    @test length(keys(broker.portfolios["test_port"].pos_handler.positions)) == 0
    SaguaroTrader.update!(broker, DateTime(2020, 1, 6, 14, 30))
    @test length(keys(broker.portfolios["test_port"].pos_handler.positions)) == 0
